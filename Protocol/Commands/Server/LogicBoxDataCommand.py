...
class LogicBoxDataCommand(Writer):
    ...
    def encode(self):
        ...
        if self.player.box_id == 3 or self.player.box_id == 9:
            self.writeVint(203) # CommandID
            self.writeVint(0)   # Unknown
            self.writeVint(1)   # Unknown
            self.writeVint(11)  # BoxID
            # Box info end
        ...
        if self.player.box_id == 6:
            if self.player.reward_trophy_road == 1:
                self.writeVint(203)
                self.writeVint(0)
                self.writeVint(1)
                self.writeVint(10)
            elif self.player.reward_trophy_road == 2:
                self.writeVint(203)
                self.writeVint(0)
                self.writeVint(1)
                self.writeVint(12)
                self.writeVint(2)
                self.writeVint(5) # gold
                self.writeScId(0, 7)
                ...
                self.writeVint(1)
                self.writeHexa('''00 00 00''')
            elif self.player.reward_trophy_road == 3:
                self.writeVint(203)
                self.writeVint(0)
                self.writeVint(1)
                self.writeVint(10)
            elif self.player.reward_trophy_road == 4:
                self.writeByte(1)
            DataBase.replaceValue(self, 'TrophyReward', self.player.reward_trophy_road + 1)
